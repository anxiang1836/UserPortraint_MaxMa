...
class PrepareData(object):
    def __init__(self,
        ...
        self._data = pd.read_csv(self.input_csv_path, sep="###_###", header=None, encoding='utf-8')
    ...
    def stop_words(self):
        ...
        file.close()
        return [word.strip() for word in stop_words_list]
    ...
    def read_and_rewrite(self):
        ...
        # 统计每个人的包含空格的query个数
        data['Query_has_space'] = data['Query_list'].map(lambda x: PrepareData.__count_query_has_space(x.split('\t')))

        # -- 清洗数据 --
        data['Query_list'] = data['Query_list'].map(PrepareData.__clean_query)

        # -- 补全缺失值 --
        tfv = TfidfVectorizer(tokenizer=PrepareData.__Tokenizer(), min_df=3, max_df=0.95, sublinear_tf=True)
    ...
    class __Tokenizer:
        ...
        def __call__(self, line):
            ...
            for query in line.split('\t'):
                ...
                # 去停用词
                word_list = [word for word in word_list if word not in PrepareData.stop_words]
